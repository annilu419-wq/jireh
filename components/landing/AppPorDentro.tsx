...
export interface FrameCarrusel {
  ...
  src?: string;
  alt?: string;
  /** Mockup honesto ilustrado (jerarquía #2 de 19 §5): pantalla dibujada en
   *  HTML/SVG que reproduce el mecanismo real, no una captura falsa. Gana al placeholder. */
  render?: () => ReactNode;
          ...
          >
            {frames.map((f, i) => (
              <div key={i} className="shrink-0 snap-center">
                ...
                >
                  {f.render ? (
                    <div className="h-full w-full overflow-hidden bg-[var(--surface)]">{f.render()}</div>
                  ) : f.src ? (
